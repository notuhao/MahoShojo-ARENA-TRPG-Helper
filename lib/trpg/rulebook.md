...
当角色尝试一个没有特定技能覆盖的行动时，就需要进行属性判定。例如，抵抗毒药需要进行体质判定，或者举起重物需要进行力量判定。在这种情况下，成功率的计算方式并非直接使用属性值，因为那会使得角色在未受训练的领域也过于强大。取而代之，我们采用一个更具挑战性的标准：`成功率 = 相应属性值 x 50%`。

这个机制确保了技能投资的价值。一个受过训练的技能（例如“运动”技能达到60%）将远比单纯依赖属性（例如敏捷60，判定成功率为60×50%=30%）进行空中闪避要可靠得多。

##### 补充说明


1.  **“智力”相关判定：**
    * 在魔法少女题材作品中，角色的“聪明才智”通常表现为两种形式：
        * **战场直觉与侦查能力：** 这被归类为 **感知 (PER)**（如`调查`技能）。
        * **对魔法理论/高科技的理解：** 这被归类为 **魔力 (MAG)**（如`神秘学`技能）。
    * 因此，逻辑和记忆类判定使用 (MAG + PER) / 2。当涉及纯粹的逻辑推理、记忆力回溯或普通学校考试时，基于魔力和感知进行判定。
...
魔法少女的能力并非一成不变的程式，而是流动的心灵力量。因此，你设计的魔装能力决定了你的“最大出力”，但在实际战斗中，你可以自由调节输出功率。

**1. MP消耗公式**
任何魔装能力的使用都需要消耗MP。MP的消耗量在设计时确定，并在使用时根据出力动态调整。
计算公式如下（向下取整，最低消耗为1点）：

$$\text{MP消耗} = \text{当前使用的[效果标签]总阶数} + \lceil \frac{\text{激活的[修正标签]总PCP成本}}{3} \rceil$$

* **效果主导：** 效果的强度（阶数）直接转化为MP消耗（1:1）。
* **修正次要：** 修正标签（如远程、范围）虽然增加PCP造价，但在MP消耗上较为低廉（约3:1）。

**2. 变频输出 (Variable Output)**
你在角色创建时花费PCP构建的是该能力的 **“全功率形态”**。在游戏中实际施法时，你可以选择进行 **“低功率释放”** 以节约MP。
* **降低阶数：** 你可以选择以低于原本设定的阶数来释放能力（例如，将原本3阶的[伤害]降低为1阶使用）。
* **关闭修正：** 你可以选择不激活某些修正标签（例如，这一次攻击我不想要[范围: 爆炸]，只对单体造成伤害）。
* **限制：** 你不能使用超过你设计时购买的最大阶数，也不能添加你未购买的修正标签。

**计算示例：**
你设计了一个名为“红莲爆裂”的能力：
* **构筑：** `[伤害] x3` (3 PCP) + `[范围: 爆炸]` (3 PCP) + `[距离: 远程]` (2 PCP) + `[元素: 火]` (1 PCP)。
* **总PCP成本：** 9点。
* **全功率MP消耗：** 3 (效果阶数) + (6 / 3) = **5 MP**。

**实战应用：**
* **情况A（全功率）：** 面对一群敌人，你释放全功率“红莲爆裂”。消耗 **5 MP**，造成3d6火属性范围伤害。
* **情况B（低耗点杀）：** 敌人只有一个且离得很远。你决定将伤害降为2阶，并关闭[范围: 爆炸]。
    * 新配置：`[伤害] x2` + `[距离: 远程]` + `[元素: 火]`。
    * 修正PCP总和：2 + 1 = 3。
    * 新消耗：2 (效果阶数) + (3 / 3) = **3 MP**。
* **情况C（极低耗牵制）：** 你只想随手丢一团火吓唬人。
    * 新配置：`[伤害] x1` + `[元素: 火]`。
    * 新消耗：1 + (1/3 向上取整为 1) = **2 MP**。
...
1.  **决定先攻：** 所有参与者进行一次敏捷（AGI）或感知（PER）判定，由GM根据战斗情况决定。根据成功等级（大成功 > 极限成功 > 困难成功 > 成功）决定行动顺序。成功等级相同时，AGI值高者优先。除非战斗状态发生剧烈变化（如有人乱入），否则无需每轮重投，以加快游戏节奏。
...
* **防御反应：** 当攻击命中时，被攻击的目标可以使用其本回合的 **1个反应动作** 进行防御。玩家必须在以下两种方式中选择其一：
    * **闪避 (Dodge):** 依赖速度完全避开攻击。进行一次 **闪避 (AGI)** 判定对抗攻击者的成功等级。
        * **获胜：** 攻击无效，伤害为0。
        * **失败：** 攻击完全命中，你受到全额伤害。
    * **格挡 (Guard):** 依赖力量和体质硬接攻击以减轻冲击。进行一次 **格斗 (STR+AGI)** 判定（如果你持有近战武器或盾牌）或 **体质 (CON)** 判定（如果你徒手）。
        * **成功：** 你成功架住了攻击。你受到的伤害减少，减少量 = $\text{强韧值} + \text{武器/防具提供的格挡值}$。
        * **失败：** 你未能完全架住，伤害不减免。
        * **注意：** 格挡通常只能应用于近战或抛射物，无法格挡无实体的攻击（除非魔装能力有特殊说明）。
...
在战斗中，MP是比HP更紧缺的资源。魔法少女必须学会在输出与续航之间寻找平衡。

**(1) MP恢复机制**

* **自然恢复：** 在非战斗状态下（日常模式或幕间），MP每小时恢复满值。
* **魔力汇聚 (Focus):** 在战斗中，你可以花费 **1个主要动作** 专注于调整体内的能量流动。进行一次 **魔力放出 (Channel)** 或 **MAG x 5%** 判定：
    * **成功：** 你从环境中汲取能量，恢复 $\lceil \text{MAG} / 10 \rceil + 1$ 点MP。
    * **大成功：** 恢复量翻倍。
    * **失败：** 你因为分心而未能聚集足够的魔力，仅恢复 1 点MP。
* **情感共鸣：** 当角色在战斗中达成“羁绊”的重大目标（如成功保护了重要之人，或击败了宿敌）时，GM可奖励恢复 1d6 点MP。

**(2) 魔力透支 (Overchannel)**

当MP不足以支付法术消耗，但情况危急时，魔法少女可以透支生命力来强行施法。
* **代价：** 强行释放法术，不足的MP将由HP抵扣，比例为 **1 MP = 2 HP**。
* **风险：** 以此方式失去的HP属于“内伤”，无法通过普通的[治疗]魔法恢复，必须在幕间休息或使用高级医疗设施才能治愈。
...
### 附录 D：通用魔法术式 (Universal Spells)（新增）

当魔法少女的成长等级达到 **“叶 (Leaf)”** 时，她不仅能使用专属魔装，还能掌握魔法国度通用的基础术式。这些术式无需花费PCP购买，只要拥有足够的魔力即可发动。

* **判定规则：** 除非特别说明，释放术式需要进行 **魔力放出 (MAG + WILL)** 技能判定。
* **通用特性：** 通用术式通常比专属魔装效率低（MP消耗略高或效果略低），但胜在灵活多变。

**通用攻击系**

| 术式名称 | 阶数 | MP消耗 | 效果描述 |
| :--- | :--- | :--- | :--- |
| **魔弹 (Mana Bolt)** | 1 | 2 | 发射纯粹的魔力弹。远程单体，造成 `1d6` 伤害。 |
| | 2 | 4 | 强化魔弹。远程单体，造成 `2d6` 伤害。 |
| | 3 | 7 | 终极魔弹。远程单体，造成 `3d6` 伤害，且具有[贯穿]效果（无视2点护甲）。 |
| **爆裂 (Burst)** | 1 | 4 | 制造小范围爆炸。指定区域半径3米内所有目标受 `1d6` 伤害。 |
| | 2 | 7 | 强化爆炸。伤害提升至 `2d6`，范围提升至5米。 |
| **追踪术 (Homing)** | - | +2 | **[附加修饰]** 施放攻击性术式时额外消耗2点MP，若命中，使攻击无法被闪避（只能格挡或用护盾抵消）。攻击方仍需进行命中判定。 |

**通用防御与辅助系**

| 术式名称 | 阶数 | MP消耗 | 效果描述 |
| :--- | :--- | :--- | :--- |
| **屏障 (Barrier)** | 1 | 3 | 创造一面浮动盾牌。为自己或接触距离内的队友提供 `1d6+2` 点临时护盾。 |
| | 2 | 6 | 强化屏障。提供 `2d6+4` 点临时护盾，且屏障存在期间免疫[击飞]效果。 |
| **治愈 (Heal)** | 1 | 4 | 基础治疗。接触目标，恢复 `1d6` HP。无法治疗“魔力透支”造成的伤害。 |
| | 2 | 8 | 强力治疗。接触目标，恢复 `2d6` HP，并移除一个物理负面状态（如流血、毒）。 |
| **束缚 (Bind)** | 1 | 4 | 召唤魔力锁链。目标需进行STR或AGI对抗判定，失败则被[束缚] 1回合。 |
| | 2 | 7 | 强力束缚。[束缚]持续1d4回合，每回合可尝试挣脱。 |
| **隐匿 (Veil)** | - | 3 | 折光立场。在静止或慢速移动时，潜行判定获得一个奖励骰。持续10分钟或直到发动攻击。 |

**通用功能系 (Utility)**
*此类术式通常在非战斗状态下使用，若在战斗中使用需消耗主要动作。*

| 术式名称 | MP消耗 | 效果描述 |
| :--- | :--- | :--- |
| **照明 (Light)** | 1 | 创造一个跟随自己的光球，照亮半径10米范围。持续1小时。 |
| **清洁 (Clean)** | 1 | 瞬间清洁目标物体或身体上的污垢、血迹和尘土。魔法少女维持优雅的必备法术。 |
| **解锁 (Unlock)** | 3 | 尝试打开一个物理或简单的电子锁。视为一次自动成功的“巧手”判定（针对普通锁）。针对魔法锁需进行MAG对抗。 |
| **幻象 (Minor Illusion)** | 3 | 在视线范围内创造一个没有声音、气味和实体的静态幻象（体积不超过一人大小）。 |
| **定位 (Locate)** | 4 | 需持有目标物品的一小部分或与之有强烈羁绊。感知大致方向和距离。 |
| **禁魔 (Silence)** | 6 | 创造一个半径3米的区域，区域内无法施放任何消耗MP小于3的法术。持续1d4轮。 |
| **传送 (Teleport)** | 10 | 瞬间移动到视线范围内的一个未被占据的位置（最大30米）。若用于战斗中闪避，视为大成功闪避，但极度消耗魔力。 |
| **占卜 (Divination)** | 5+ | 请求GM给予一个关于未来的模糊提示或谜语。消耗5点MP，每额外消耗5点，提示稍微清晰一点。 |