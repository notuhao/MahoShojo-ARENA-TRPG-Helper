...
* **[伤害] (1 PCP/阶):** 造成伤害。每阶提供1d6伤害。
* **[治疗] (2 PCP/阶):** 恢复HP。每阶恢复1d6 HP。
* **[护盾] (2 PCP/阶):** 创造一个能吸收伤害的临时护盾。每阶提供1d8点护盾值。
* **[束缚] (3 PCP):** 限制目标的移动。
* **[强化] (2 PCP/阶):** 暂时提升一个属性或技能。
* **[幻象] (4 PCP):** 创造一个虚假的感官影像。
* **[创造] (5 PCP):** 凭空创造简单的无生命物体。

**修正标签 (Modifier Tags):** 修改效果的作用方式。

* **[距离: 远程] (+2 PCP):** 效果可以作用于视线内的目标。
* **[范围: 爆发] (+3 PCP):** 效果影响一个小范围区域内的所有目标。
* **[持续: 专注] (+2 PCP):** 效果只要你花费行动维持就会一直持续。
* **[元素: 火/冰/雷等] (+1 PCP):** 为效果附加一种元素属性，可能造成额外效果或被特定防御克制。
...
| 状态效果 | 机制 | 持续时间 |
| :--- | :--- | :--- |
| **束缚 (Bound)** | 无法移动。闪避判定受到一个惩罚骰。 | 直到被解放或通过一次STR判定。 |
| **燃烧 (Burning)** | 每回合开始时受到1d6点火焰伤害。 | 直到花费一个主要动作扑灭火焰。 |
| **眩晕 (Stunned)** | 无法执行任何动作。 | 直到该角色下一回合结束。 |
| **混乱 (Confused)** | 随机行动（攻击最近的人，或什么都不做）。 | 直到通过一次WILL判定。 |
| **影染 (Shadow-touched)** | 无法使用光辉值呼唤羁绊。共情判定受到一个惩罚骰。 | 直到光辉值恢复到一半以上。 |
...
在激烈的冒险之间，角色们需要时间来处理伤痛、加深感情和为下一次任务做准备。幕间休息阶段为玩家提供了结构化的“日常模式”选项。

* **加深羁绊：** 选择一个羁绊对象，进行一次专属的角色扮演场景。成功的互动可以强化该羁绊，例如，在所有与该羁绊相关的判定中获得永久的+5%加成。
* **恢复光辉：** 描述你的角色如何通过爱好、与朋友相处或享受平静生活来放松。进行一次魅力（CHM）或意志（WILL）判定，成功则恢复1d6点光辉值。
