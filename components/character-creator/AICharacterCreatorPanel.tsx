...
2.  **魔力值与消耗 (基于规则书新增内容):**
    - 最大 MP = ceil(MAG / 4)。
    - 施放能力的 MP 消耗 = 当前使用的效果阶数 + ceil(激活的修正标签PCP总和 / 3)。
    - 请在每个能力的 description 末尾追加一行 "【MP参考】满功率:X MP；低功率(仅核心效果):Y MP；公式:阶数+ceil(修正/3)"，并确保X、Y按上式计算（低功率使用1阶且不启用修正）。
3.  **内容完整性:** 你必须为角色卡的所有叙事字段（info, magicConstruct, wonderlandRule, blooming, gemScepter, bonds）提供富有想象力且符合角色设定的内容。
4.  **JSON结构:** 你必须严格按照下面第4点提供的 **JSON输出结构 (Schema)** 格式返回一个JSON对象。
...
5.  **最具创造性的一步:** 设计2-3个总成本恰好为20 PCP的“心之花”能力。为它们取名，并组合效果与修正标签，使其与角色设定相符。参考效果与修正，为每个能力添加一段生动的 'description'，描述其具体效果与作用方式；在描述末尾追加满功率与低功率的 MP 参考行（公式：阶数+ceil(修正/3)），保证与上方MP规则一致。
