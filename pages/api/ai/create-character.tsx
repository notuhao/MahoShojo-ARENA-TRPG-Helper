 ...
 * @description
 * - [新增] 接收 `allowCustomSkills` 和 `allowCustomPowers` 参数。
 * - [新增] 根据参数动态修改系统提示词，授权AI进行内容创造。
 * - [修改] 使用新的 `aiGeneratedCharacterSchema` 作为返回数据结构，以接收AI创造的自定义内容。
 */
...
export default async function handler(req: NextRequest) {
  ...
  try {
    ...
    const generationConfig: GenerationConfig<AIGeneratedCharacterData, { prompt: string }> = {
      ...
      schema: aiGeneratedCharacterSchema, // 【修改】使用新的顶层Schema
      temperature: 0.8,
      maxTokens: 4096,
      taskName: 'TRPG角色创建',
